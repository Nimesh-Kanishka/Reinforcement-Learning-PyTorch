...
class Agent:
    def __init__(self,
                 ...
                 theta: float = 0.15,
                 sigma: float = 0.2,
                 network_conv_layer_params: Sequence[dict] | None = None,
                 network_fc_layer_sizes: Sequence[int] | None = None,
                 use_gpu: bool = False,
        ...
        self.save_folder = f"DDPG/DDPG_Pixel_Inputs/{env_id}" if save_folder is None else save_folder

        self.env = DynamicRenderEnv(env_id, frame_stack_size, **env_kwargs)

        self.device = torch.device("cuda" if use_gpu and torch.cuda.is_available() else "cpu")

        self.actor = ActorNetwork(self.env.observation_space.shape, self.env.action_space.shape[0],
                                  network_conv_layer_params, network_fc_layer_sizes).to(self.device)
        self.actor_optimizer = Adam(self.actor.parameters(), lr=actor_lr)

        self.critic = CriticNetwork(self.env.observation_space.shape, self.env.action_space.shape[0],
                                    network_conv_layer_params, network_fc_layer_sizes).to(self.device)
        self.critic_optimizer = Adam(self.critic.parameters(), lr=critic_lr)

        self.target_actor = ActorNetwork(self.env.observation_space.shape, self.env.action_space.shape[0],
                                         network_conv_layer_params, network_fc_layer_sizes).to(self.device)
        
        self.target_critic = CriticNetwork(self.env.observation_space.shape, self.env.action_space.shape[0],
                                           network_conv_layer_params, network_fc_layer_sizes).to(self.device)
    ...
    def train(self,
        ...
        action_noise = OUActionNoise(mu=np.zeros(self.env.action_space.shape[0]),
                                     theta=self.theta, sigma=self.sigma)
        ...
        for episode in pbar:
            ...
            while not episode_done:
                ...
                if global_step > train_start_step:
                    observation_batch, action_batch, reward_batch, next_observation_batch, done_batch = \
                        replay_buffer.sample(self.minibatch_size)
            ...
            if (global_step > train_start_step) and (avg_reward >= save_start_reward):
